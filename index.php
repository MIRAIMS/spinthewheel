...
if (isset($_SESSION['result']) && isset($_POST['resultValue']) && !empty($_POST['resultValue'])) {
	$value = $_POST['resultValue'];
	$result = $_SESSION['result'];
	//print_r($value.' '.$result[1]);
	...
	$get_range = 'A'.($result[0] + 1).':J'.($result[0] + 1);
	$row = getData($service,$get_range,$spreadsheetId)[0];
	//print_r($row);
	$count = $row[9];
	switch($count) {
		case 0: $_SESSION['counts'] = 1;	$scoreCell = 'D';
			$timeStamp = 'E';
			break;
		case 1: $_SESSION['counts'] = 2;	$scoreCell = 'F';
			$timeStamp = 'G';
			break;
		case 2: $_SESSION['counts'] = 3;	$scoreCell = 'H';
			$timeStamp = 'I';
			break;
		default: $_SESSION['counts'] = 3;
				 break;
	}
	
	if ($count < 3) {
		//updateing values
		$update_range = $scoreCell.''.($result[0] + 1).':I'.($result[0] + 1); 
		...
		UpdateData($service,$update_range,$spreadsheetId,$values);
		// updating count
		$update_range = 'J'.($result[0] + 1).':J'.($result[0] + 1); 
...
<html>
  ...
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.min.js"></script>
	...
	<div class="container">
    <img src="logo.png" width="200" style="position:absolute;left: 100px; top: 0px;"/>
        <div align="center" style="padding-top: 100px;" >
        <h2>Spin to Win </h2><br>
        <div>You have <span></span>chances to spin the wheel and try your luck.</div><br>
		<?php 
			if (isset($_SESSION['result'])) {
				echo '<h3>Welcome '.$_SESSION['result'][1].'!</h3>';
			} else {
				echo '<h3>Enter the code!</h3>';
			}
		?>
		...
		<div class="col-sm-12 text-center">
			<h6>Note: Your spinwheel responses will be recorded. Your team will reach out to you internally.</h6>
		</div>
	</div>
		
				<form id="submitResult" method="POST" action="" class="d-none">
				  <div class="form-group">
					<input type="hidden" class="form-control" id="resultValue" name="resultValue">
				  </div>
				</form>
        ...
        <script>
            ...
            function alertPrize(indicatedSegment)
            {
				...
				$('#resultValue').val(indicatedSegment.text);
